...
import { assessDrillWithAI, assessReflection, gradeInterview } from './assessment.ts';
...
async function api(req:IncomingMessage,res:ServerResponse,url:URL){
  ...
  if(match){const {user}=requireUser(req),attempt=getAttempt(match[1],user.handle);if(!attempt)throw new HttpError(404,'Attempt not found.');const action=match[2];
    ...
    if(method==='POST'&&action==='submit'){if(attempt.status!=='active')throw new HttpError(409,'This attempt is already finished.');if(attempt.mode==='rated'&&Date.now()>Date.parse(attempt.deadline))throw new HttpError(409,'The rated attempt time has expired.');const input=await body(req);
      if(attempt.kind==='drill'){
        const answer=text(input.answer??attempt.answer,20_000,'answer');saveDraft(attempt,{answer});rate(req,`drill-grade:${user.handle}`,24,60*60_000);
        const normalized=answer.trim().replace(/\s+/g,' ').toLowerCase();
        const reusedAnswer=normalized.length>0&&attemptsFor(user.handle).some(other=>other.id!==attempt.id&&other.kind==='drill'&&other.status==='submitted'&&other.answer.trim().replace(/\s+/g,' ').toLowerCase()===normalized);
        const result=await assessDrillWithAI(attempt.challengeId,answer,{reusedAnswer});if(!result)throw new HttpError(404,'Challenge not found.');finishAttempt(attempt,result);return json(res,200,{result,attempt:getAttempt(attempt.id,user.handle)});
      }
      const code=text(input.code??attempt.code,40_000,'code'),reflection=text(input.reflection??attempt.reflection??'',8_000,'reflection');saveDraft(attempt,{code,reflection});const challenge=getPrivateInterview(attempt.challengeId)!;recordRun(attempt.id);const executed=await runCode(code,challenge.tests.map(t=>({id:t.id,input:t.input,expected:t.expected,hidden:t.hidden,label:t.label})));const grade=gradeInterview(attempt.challengeId,executed.map(x=>({testId:x.id,passed:x.passed})))!;const result={...grade,totalScore:Math.round(100*grade.score/Math.max(1,grade.maxScore)),reflection:assessReflection(reflection)};finishAttempt(attempt,result);return json(res,200,{result:getAttempt(attempt.id,user.handle)?.result});}
